...
def comparison_boxfigure( names, results, score_type):
    ...
    else: fig.suptitle('Accuracy Comparison')
...
def comparison_barfigure(results, score_type ):
    ...
    # Make the plot
    plt.bar(r1, bars1, color='#7f6d5f', width=barWidth, edgecolor='white', label='NB')
    plt.bar(r2, bars2, color='#557f2d', width=barWidth, edgecolor='white', label='LR')
    plt.bar(r3, bars3, color='#2d7f5e', width=barWidth, edgecolor='white', label='SVM')
...
def cleaned_to_dataframe(csvFileName):
    ...
    new_trainDF = pd.concat([new_cyberbullying, non_cyberbullying]).sort_index()
...
def dataset_preparation(cleanedFileName, features = 0 ):
    ...
    train_x, valid_x, train_y, valid_y = model_selection.train_test_split(trainDF['tweet'], trainDF['cyberbullying'])
    ...
    if features != 0:
        count_vect = CountVectorizer(analyzer='word', token_pattern=r'\w{1,}', max_features = features)  
        tfidf_vect = TfidfVectorizer(analyzer='word', token_pattern=r'\w{1,}', max_features=features) 
        tfidf_vect_bigram = TfidfVectorizer(analyzer='word', token_pattern=r'\w{1,}', ngram_range=(2,2), max_features = features)
        tfidf_vect_trigram = TfidfVectorizer(analyzer='word', token_pattern=r'\w{1,}', ngram_range=(3,3), max_features = features)
    else: 
...
if __name__ == '__main__':
    ...
    dataset, feature_getters = dataset_preparation(cleanedFileName, feature_num)
    ...
    count_trained_classifiers = models.get_trained_models_from(dataset['xtrain_count'], dataset['train_y'])
    word_tfidf_trained_classifiers = models.get_trained_models_from(dataset['xtrain_tfidf'], dataset['train_y'])
    bigram_tfidf_trained_classifiers = models.get_trained_models_from(dataset['xtrain_tfidf_bigram'], dataset['train_y'])
    trigram_tfidf_trained_classifiers = models.get_trained_models_from(dataset['xtrain_tfidf_trigram'], dataset['train_y'])
    
    ### get Accuracy score ###
    count_accuracy_scores = models.get_scores_from(count_trained_classifiers, dataset['xvalid_count'], dataset['valid_y'], "accuracy")
    word_tfidf_accuracy_scores = models.get_scores_from(word_tfidf_trained_classifiers, dataset['xvalid_tfidf'], dataset['valid_y'], "accuracy")
    bigram_tfidf_accuracy_scores = models.get_scores_from(bigram_tfidf_trained_classifiers, dataset['xvalid_tfidf_bigram'], dataset['valid_y'], "accuracy")
    trigram_tfidf_accuracy_scores = models.get_scores_from(trigram_tfidf_trained_classifiers, dataset['xvalid_tfidf_trigram'], dataset['valid_y'], "accuracy")    
    
    ### get F1 score ###
    count_f1_scores = models.get_scores_from(count_trained_classifiers, dataset['xvalid_count'],  dataset['valid_y'], "f1")
    word_tfidf_f1_scores = models.get_scores_from(word_tfidf_trained_classifiers, dataset['xvalid_tfidf'],  dataset['valid_y'], "f1")
    bigram_tfidf_f1_scores = models.get_scores_from(bigram_tfidf_trained_classifiers, dataset['xvalid_tfidf_bigram'],  dataset['valid_y'], "f1")
    trigram_tfidf_f1_scores = models.get_scores_from(trigram_tfidf_trained_classifiers,  dataset['xvalid_tfidf_trigram'],  dataset['valid_y'], "f1")
    
    
    ### get confusion matrix  ###
    count_conf_matrix = models.get_confusion_matrix_from(count_trained_classifiers, dataset['xvalid_count'],  dataset['valid_y'])
    unitfidf_conf_matrix = models.get_confusion_matrix_from(word_tfidf_trained_classifiers, dataset['xvalid_tfidf'],  dataset['valid_y'])
    bitfidf_conf_matrix = models.get_confusion_matrix_from(bigram_tfidf_trained_classifiers, dataset['xvalid_tfidf_bigram'],  dataset['valid_y'])
    tritfidf_conf_matrix = models.get_confusion_matrix_from(trigram_tfidf_trained_classifiers, dataset['xvalid_tfidf_trigram'],  dataset['valid_y'])
    ...
    classifiers = [ count_trained_classifiers, word_tfidf_trained_classifiers,  bigram_tfidf_trained_classifiers , trigram_tfidf_trained_classifiers ]
    acc_scores = [count_accuracy_scores, word_tfidf_accuracy_scores,  bigram_tfidf_accuracy_scores, trigram_tfidf_accuracy_scores]
    f_scores = [count_f1_scores, word_tfidf_f1_scores, bigram_tfidf_f1_scores, trigram_tfidf_f1_scores]
    ...
    for featureindex in range(4):
        for modelindex in range(3):
            check_results.append(models.check_model_with_sent(sent, classifiers[featureindex][modelindex], feature_getters[featureindex]))
